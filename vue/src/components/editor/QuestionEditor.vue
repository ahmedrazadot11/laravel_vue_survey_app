<template>
    ...
    <div class="grid gap-3 grid-cols-12">
        ...
        <div class="mt-3 col-span-3">
            <label :for="'question_type_'+ model.data" class="block text-sm font-medium text-gray-700">Select Question Type</label>
    ...
    <div class="mt-3 col-span-3">
        <label :for="'question_descripiton_'+ model.id" class="block text-sm font-medium text-gray-700">Description</label>
        <textarea :name="'question_descripiton_' + model.id" v-model="model.description" @change="dataChange" :id="'question_description_'+model.id" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 blcok w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
    </div>
    ...
    <div>
        <div v-if="shouldHaveOptions()" class="mt-2">
            ...
            <div v-for="(option, index) in model.data.options" :key="option.uuid" class="flex items-center mb-1">
                ...
                <button @click="removeOption(option)" class="h-6 w-6 rounded-full flex items-center justify-center border border-transparent transition-colors hover:border-red-100">
